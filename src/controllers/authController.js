...
export const register = async (req, res) => {
  try {
   ...
   //20/03 añado age y role
    const { email, password, name, surname, age, role } = req.body;
    ...
    const newUser = new User({ email, password: hashedPassword, name, surname, age, role, status: 1 });
    await newUser.save();

    // Generar un token de acceso y lo guardo en un token seguro (httpOnly) //19/03 incluyo el role en el token
    const accessToken = jwt.sign({ id_user: newUser.id_user, name: newUser.name, age: newUser.age, role: newUser.role}, process.env.JWT_SECRET);
